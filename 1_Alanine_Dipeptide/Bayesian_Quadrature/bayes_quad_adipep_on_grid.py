...
from helper_functions import QuadratureProductMatern52LebesgueMeasure, SumRBFWhiteGPy,SumMatern52WhiteGPy,write_result,get_ground_truth,integration_2D_rgrid,make_fes_figure,plot_acquisition_function
import matplotlib.pyplot as plt

def get_force(phi_value, psi_value):
    """
    Interpolate derivative of free energy from metadynamics data.
    
    Returns array: [dF/dphi, dF/dpsi]
    """
    x_der = dx_interpolated((phi_value,psi_value))
    y_der = dy_interpolated((phi_value,psi_value))
...
# Interpolators for derivative evaluation at arbitrary points
dx_interpolated = RegularGridInterpolator((x_grid,y_grid), x_derivative.T) 
dy_interpolated = RegularGridInterpolator((x_grid,y_grid), y_derivative.T)
...
for init_phi, init_psi in zip(initial_positions_phi, initial_positions_psi):
    derivatives.append(get_force(init_phi, init_psi))
...
for i in range(1,queries+1): #10
    ...
    force_x, force_y = get_force(new_x_ivr, new_y_ivr)    
