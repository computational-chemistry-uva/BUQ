...
# generate a seed in a chosen range
...
def run_command(command: str):
    ntasks = os.environ.get("SLURM_NTASKS", "48")
    setup_cmds = (
        f"srun -n {ntasks} "
        f"~/software/lammps_ice/build/lmp {command} &> lmp.out"
    )
    env = os.environ.copy()
    subprocess.run(
        setup_cmds,
...
task_id = task_id - 1  # Adjust for 0-based indexing
# Read CSV manually
...
#stable points 1.6 and 284.9
init_es = np.array([0.627543, 2.69581433, 282.5571406333334,  287.060134])
