...
task_id -=1
precommand = "gmx mdrun -s simulations_essentials/md.tpr " #for running on a cluster
...
#Simulation setup
ns = 0.02
#kappa =200 normal value is kappa 200
measure_after_ps = 10 #this is above the maximum steering time, which is around 30 ps (2(*pi nee geen pi vgm -1,tot 1 dus 2) * 1000 + 1500)*0.002 = 30 ps
nsteps = 500000 * ns 
nsteps = int(nsteps)  
...
name = f"AD_{kernel_type}kappa_{kappa}_ls_{lengthscale}_w_{weight_acq_fes}_n{noise}_acq_{acq_function}_q_{queries}"
...
for init_phi, init_psi in zip(initial_positions_phi, initial_positions_psi):
    ...
    run_command(command)
    derivatives.append(get_force(init_phi, init_psi, kappa_phi=kappa, kappa_psi=kappa, measure_after_ps=measure_after_ps))
