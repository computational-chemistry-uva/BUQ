...
def get_metadynamics( derivatives=False):
    metafile = open("simulations_essentials/fes_final_old.dat")
...
for n_points in range(1,total_n_points+1):  
    ...
    for x_sample in sampling_points_x:
        for y_sample in sampling_points_y:
            ...
            run_command("rm *#*")
            command = precommand +"-plumed  Colvars/plumed_{:.3f}_{:.3f}.dat -nsteps {} -x Colvars/traj_{:.3f}_{:.3f}.xtc".format(x_sample, y_sample,nsteps,x_sample, y_sample)
            run_command(command)
